...
public class DragDropRankEditorGUI {
    ...
    private void renderAll(Inventory inv) {
        ...
        inv.setItem(49, makeBtn(Material.LIME_DYE,  "§a§lSave",    "§7Save slot changes to ranks.yml"));
        inv.setItem(53, makeBtn(Material.BARRIER,   "§c§lClose",   "§7Close without saving"));
    }
    ...
    public void handleClick(Player player, int rawSlot) {
        ...
        if (rawSlot == 49) { handleSave(player);        return; }
    ...
    private void moveRank(Player player, Inventory inv, String rankId, int newSlot) {
        ...
        player.sendMessage("§aMoved §e" + rank.getDisplayName() + " §a→ slot §e" + newSlot);
        plugin.getSoundManager().playClick(player);
    }
    ...
    private void handleSave(Player player) {
        ...
        plugin.getRankYamlManager().saveAsync(() ->
                player.sendMessage("§a✔ Slot layout saved to ranks.yml!"));
    }
