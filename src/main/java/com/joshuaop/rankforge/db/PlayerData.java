...
public record PlayerData(
    ...
    public static PlayerData defaultData(UUID uuid, String playerName, String defaultRank) {
        return new PlayerData(uuid, playerName, defaultRank, 0L, 0.0, "en", 0L);
    }

    // ── Functional copies ─────────────────────────────────────────────────────

    public PlayerData withRank(String newRankId) {
        return new PlayerData(uuid, playerName, newRankId, experience, money, language, blockBreaks);
    }

    public PlayerData withLanguage(String newLang) {
        return new PlayerData(uuid, playerName, rankId, experience, money, newLang, blockBreaks);
    }

    public PlayerData withMoney(double newMoney) {
        return new PlayerData(uuid, playerName, rankId, experience, newMoney, language, blockBreaks);
    }

    public PlayerData withExperience(long newExp) {
        return new PlayerData(uuid, playerName, rankId, newExp, money, language, blockBreaks);
    }

    public PlayerData withPlayerName(String newPlayerName) {
        return new PlayerData(uuid, newPlayerName, rankId, experience, money, language, blockBreaks);
    }
