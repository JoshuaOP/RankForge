 ...
 * Changes are applied to the in-memory RankModel and synced to ranks.yml.
 */
public class RankEditor {
    ...
    /**
     * Persist all in-memory ranks to ranks.yml asynchronously.
     * @param onComplete optional callback run on the main thread when done.
     */
    public void save(Runnable onComplete) {
        plugin.getRankYamlManager().saveAsync(onComplete);
    }
