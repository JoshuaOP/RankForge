 ...
 *   <li>{@code GET /api/leaderboard}        — top-10 leaderboard.</li>
 * </ul>
 *
...
public class RestAPIServer {
    ...
    private String route(String path) {
        ...
        if (path.equals("/api/leaderboard"))return buildLeaderboard();
    ...
    private String buildLeaderboard() {
        ...
        StringBuilder sb = new StringBuilder("[");
        boolean first = true;
        for (LeaderboardManager.LeaderboardEntry e :
                plugin.getLeaderboardManager().getTop(10)) {
            if (!first) sb.append(",");
            first = false;
            sb.append("{")
              .append("\"position\":").append(e.position()).append(",")
              .append("\"player\":\"").append(escape(e.playerName())).append("\",")
              .append("\"rank\":\"").append(escape(e.rankId())).append("\",")
              .append("\"experience\":").append(e.experience())
              .append("}");
        }
        sb.append("]");
        return sb.toString();
